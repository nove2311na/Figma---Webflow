...
def resolve_variable(val, variables):
    ...
    if val.startswith("var(") and val.endswith(")"):
        ...
        if var_name in variables:
            ...
            # If size unit is present, append it
...
class FigmaHTMLRewriter(HTMLParser):
    ...
    def match_classes(self, style_dict):
        ...
        def is_native_selector(sel):
            ...
            if clean.startswith("."):
                clean = clean[1:]
            return clean.startswith("w-") or clean.startswith("wf-") or f".{clean}" in self.webflow_native or clean in self.webflow_native
        ...
        def is_html_tag_selector(sel):
            ...
            return clean in html_tags or f".{clean}" in self.native_elements or clean in self.native_elements

        for cname, c_meta in self.classes.items():
            # Block matching against Webflow native classes or HTML tag selectors
            if is_native_selector(cname) or is_html_tag_selector(cname):
                continue
...
def main():
    ...
    mapping_path = Path(".claude/skills/[new]-figma-to-html-architect/references/html-semantic-mapping.json")
