...
def main():
    ...
    parser.add_argument("--guide", default=".claude/skills/[new]-design-system-sync/references/.user-figma-setup.md")
    parser.add_argument("--schema", default=".claude/skills/[new]-design-system-sync/schema/figma-design-system-contract.schema.json")
    ...
    if mapping_path.exists() and baseline_path.exists():
        try:
            ...
            # Helper to check class selector formats
            def is_native_selector(sel):
                # Check for .w-* or w-*
                clean = sel.strip()
                if clean.startswith("."):
                    clean = clean[1:]
            ...
            for f_var, w_var in var_map.items():
                ...
                if is_native_selector(w_var) or w_var.startswith("--w-") or w_var.startswith("--wf-"):
                    report["errors"].append(f"Baseline Violation: Figma variable '{f_var}' maps to Webflow native variable '{w_var}'.")
                    violations += 1
                if is_html_tag_selector(w_var):
