...
def validate_project_library(root: Path, site_id: str) -> list[str]:
    ...
    try:
        lib_dir = resolve_library(root, site_id).parent
    except Exception:
        lib_dir = _library_dir(root, site_id)

    # 2. Library dir exists
    if not lib_dir.is_dir():
        failures.append(f"library dir missing: {lib_dir.relative_to(root).as_posix() if lib_dir.is_relative_to(root) else lib_dir}")
